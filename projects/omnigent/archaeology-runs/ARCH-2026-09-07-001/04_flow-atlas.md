...
用户认证（server/auth.py + accounts_*）→ 会话权限（SqlSessionPermission，db_models.py）
  ...
  → 裁决回传：approval 事件 POST /v1/sessions/{id}/events → pending_approvals.resolve（幂等）
  → 拒绝可见性：DENY 文本作为 tool output 返回（LLM 看到拒绝）
```
关键 symbol：`evaluate_policy=True`、`pending_approvals.register/cleanup/resolve`、`_DEFAULT_WAIT_SECONDS=86400`。

...
> Job: `ARCH-2026-09-03-001` | 层：Generalized Knowledge（窄尖顶，L3/L4）| 8 个 KO（**Reconciled**）
...
> **Reconciliation**：KO-03 scope 已按独立验证收紧；KO-01/02/04/06/08 簇并入 Reconciliation 新增 EK（EK-32/30/29/27/31）；全部簇平均规模 5.0，可回溯 EK/EV。
...
- **aggregation_rule**:
  ...
  - cluster_eks: [EK-01, EK-02, EK-03, EK-06, EK-32]
  - edges: causal EK-01→EK-02→EK-03；EK-06 约束 EK-03 的格式演进；EK-32（legacy 硬拒绝）与 EK-06 同属格式演进纪律
  - naming: "回合循环 = 日志边界"——所有 EK 共享"先写日志再判定"的因果链
  - scope_expansion: 任一成员 EK 只描述单点（循环/日志/持久化/版本/兼容），KO-01 断言三者组合构成"可恢复循环"这一整体性质
- **derivation.facts**: [EK-01, EK-02, EK-03, EK-06, EK-32]
...
- **aggregation_rule**:
  ...
  - cluster_eks: [EK-23, EK-16, EK-12, EK-13, EK-21, EK-22, EK-30]
  - edges: mechanism EK-23↔EK-16↔EK-12↔EK-13↔EK-21↔EK-22↔EK-30（同为 seam 三件套实例；EK-22 为 subagent seam 的外部 provider 家族；EK-30 settings seam）
  - naming: "三件套"模式在 llm/sandbox/credentials/subagent/settings（含外部 agent provider）多个独立子系统重复出现
  - scope_expansion: EK-23 描述机制本身，KO-02 断言"多系统各自独立演化仍保持统一可替换契约"的系统级性质
- **derivation.facts**: [EK-23, EK-16, EK-12, EK-13, EK-21, EK-22, EK-30]
...
## KO-03 — 权限系统的 Fail-Closed 不变量族（R3 不变量簇）[R：scope 收紧]

- **claim**：在**受限执行路径**（受控工具调用）上，Agent 权限体系把"默认拒绝"实现为多个**独立层叠的 fail-closed 不变量**——审批非 allowed-once 即拒绝、工具守卫只有拒绝没有允许、collapse 调用在策略管线前确定性拒绝、沙箱三态非允许即受限、凭证空值即不存在——**任意一层失败都不产生授权**。非受限路径（如 plan-mode 只读投影）不适用本族。
...
- **aggregation_rule**:
  ...
  - cluster_eks: [EK-10, EK-09, EK-07, EK-13, EK-28]
  - edges: constraint/mechanism EK-09→EK-07；EK-10↔EK-09（fail-closed 家族）；EK-28（collapse 前置拒绝）为 EK-07 管线之前的更早确定性拒绝；EK-13 独立层
  - naming: 多层都指向同一不变量"默认拒绝，失败=拒绝"
  - scope_expansion: 任一 EK 只覆盖单层，KO-03 断言"层叠安全"（defense in depth）系统性质
- **derivation.facts**: [EK-10, EK-09, EK-07, EK-13, EK-28]
- **flow_traceability**: F-05（Authority）collapse→审批→守卫→沙箱→凭证
- **scope**: applies_when Agent 执行不可信输入、持敏感能力；does_not_apply_when 无权限语义的只读工具 / 非受限只读路径
- **validation**: truth PASS | abstraction OK | counterexample found→已条件化（见 C-06 danger-full-access；独立验证进一步收紧 scope）| blind_reconstruction performed
...
- **aggregation_rule**:
  ...
  - cluster_eks: [EK-02, EK-04, EK-05, EK-18, EK-19, EK-20, EK-29]
  - edges: EK-02→EK-04→EK-19（causal/constraint）；EK-05 与 EK-19 共享投影/事件机制；EK-29（plan-mode per-agent 协作状态）是投影层用于协作/恢复的实例；EK-18/EK-20 为上下文组装与回合结果约束
  - naming: 七条 EK 覆盖"事实→可见→派生→压缩→协作恢复"记忆生命周期的互补维度
  - scope_expansion: 任一 EK 只描述一层机制，KO-04 断言四层如何协同且互不越权
- **derivation.facts**: [EK-02, EK-04, EK-05, EK-18, EK-19, EK-20, EK-29]
...
- **aggregation_rule**:
  ...
  - cluster_eks: [EK-10, EK-11, EK-14, EK-12, EK-27]
  - edges: causal EK-10→EK-11→EK-14→EK-12（决策→策略→预设→执行边界）；EK-27（delegation 委派继承）使策略随权威边界跨子会话传递，闭环闭合
  - naming: 与 Policy Flow（F-07）同构的标准治理链
  - scope_expansion: 从单点审批上升到"策略如何自我固化并约束未来"的闭环
- **derivation.facts**: [EK-10, EK-11, EK-14, EK-12, EK-27]
...
- **aggregation_rule**:
  - rule: **R4**（主题簇：同一主题互补维度）
  - cluster_eks: [EK-24, EK-23, EK-25, EK-26, EK-31]
  - edges: EK-24→EK-23（dependency）；EK-25→EK-26（constraint）；EK-26→EK-24（constraint）；EK-31（repeat-tool-reminder advisory 检测器）为运行时反馈型护栏
  - naming: 五 EK 覆盖"组装方式→图治理→运行时自检→启动边界→运行时反馈"五个互补维度
  - scope_expansion: 从各自护栏上升到"护栏组合如何让可插拔系统保持可治理"整体性质
- **derivation.facts**: [EK-24, EK-23, EK-25, EK-26, EK-31]

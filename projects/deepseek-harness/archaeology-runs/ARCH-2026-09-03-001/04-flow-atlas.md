...
## F-05 · Authority Flow — 每一步谁有执行权？
- **question**：一次工具调用/文件写入/子进程，如何获得并受制于执行权？
- **chain**:
  1. 工具调用 → `tools/execute`（`core/tools/src/index.ts:227`）— 入口
  2. `tools/pre-execute` + guards（`tools/index.ts:144,697`）— **单调拒绝**守卫（guardReason 取第一个拒绝）
  3. `ctx.approval.request`（`interaction/user-approval/src/index.ts:207`）— Answerer 链，fail-closed `unavailable`；`never` 政策在 dispatch 前确定性拒绝（`index.ts:258-264`）
  4. `sandbox.confine`（`sandbox/sandbox/src/index.ts`）— per-call 模式（read-only/workspace-write/danger-full-access）约束子进程 argv
  5. `credentials.resolve`（`credentials/credentials/src/index.ts:2`）— 每操作重解析 env 引用
  6. `tools/post-execute`（`tools/index.ts:167`）— 结果层复核/改写
- **gates**: `ApprovalOutcome` 非 `allowed-once` 即拒绝；守卫无 allow 语义；sandbox partial enforcement 作为上报事实
- **evidence**: EV-011..021
...
- 结构 `产生→守卫→审批→执行→记录` 在 F-01/F-05/F-06 反复出现 → **Observed repeated structure**（已由 KO-03/KO-05 承载）

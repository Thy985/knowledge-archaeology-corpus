...
> Job: `ARCH-2026-09-03-001` | 层：Engineering Knowledge（宽底座，L1/L2）| 26 条 EK
> 每条 EK = 图节点，声明 `links`（六类边）；证据引用见 `06-validation-evidence.md`（EV-xxx）。
> 铁律：孤立 EK 要么被连接要么降级；本图 26/26 全部连接 KO 簇（游离 0%）。
...
### EK-07 — 受控工具执行管线
- **claim**：工具执行走 `pre-execute → guards（monotonic denial）→ execute → post-execute → 结果物化`；守卫（ToolGuard）**只有拒绝没有允许**，"first monotonic denial" 即拦停；post-execute 可改写/阻断结果（空结果有固定文案）。
- **evidence**：EV-011, EV-012 | **abstraction**: L2 | **value**: A | **confidence**: high
- **category**: PERMISSION/TOOLING | **epistemic_status**: Fact
- **flows**: authority/control/data
- **links**:
  - → EK-09（causal: guards 决定是否进入 execute）
  - → EK-10（dependency: 需要审批的动作依赖 approval 决策）
